...
if ($_POST) {
  ...
  foreach($game_json->objective_defines AS $objective) {
    if (!empty($wiki_campaign) && $objective->campaign_id == $wiki_campaign->id) {
      $objectives[$objective->id] = $objective;
    }
  }

  $achievements = array();
  foreach($game_json->achievement_defines AS $achievement) {
  ...
  foreach ($achievements AS $achievement) {
    ...
    if (!empty($achievement->description)) {
      ...
      $event_achievements .= $achievement->name . ' by completing "' . $objectives[$objective_to_complete]->name . "\".\n\n{{IncDPS|All|1%}}\n\n";
