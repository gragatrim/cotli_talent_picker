...
Time to complete FP: <input type="text" name="time_to_complete_fp" value="<?php echo (isset($user->time_to_complete_fp) ? $user->time_to_complete_fp : 0); ?>"><br>
Time to complete sprint: <input type="text" name="time_to_complete_sprint" value="<?php echo (isset($user->time_to_complete_sprint) ? $user->time_to_complete_sprint : 0); ?>"><br>
...
<table>
  ...
  for ($i = 500; $i <= 10500; $i += 500) {
    echo '<tr class="' . ($dungeon_results[$i]['idol_over_fp'] > 0 ? 'green' : 'red') . '">
            ...
            <td>' .  $dungeon_results[$i]['idols_gained'] . '</td>
            <td>' .  $dungeon_results[$i]['idols_per_hour'] . '</td>
            <td>' .  $dungeon_results[$i]['idols_per_fp_time'] . '</td>
            <td>' .  $dungeon_results[$i]['idol_over_fp'] . '</td>
          </tr>';
  }?>
</table>
